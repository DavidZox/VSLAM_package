...
- [x] **`./colcon_build.sh` 已經在真正的 ROS2 Jazzy 容器裡完整跑過,四個套件(`g2o` → `stella_vslam`
      → `stella_vslam_ros` → `vslam_bringup`)一次全部編譯成功**,`ros2 launch vslam_bringup
      stereo_vslam.launch.py --show-args` 也確認 launch 檔本身沒問題、參數都正確帶出來。過程中發現並
      修掉三個真實 bug(見下方「已知眉角」):g2o fork 的 CMake config 問題、`stella_vslam_ros` 的
      ament 環境變數 hook 重複註冊、`cv_bridge.h`→`cv_bridge.hpp` 改名(ROS2 Jazzy 才會踩到)
- [ ] 還沒接上真的 ZED 相機/`zed-ros2-wrapper` 實際跑過完整流程(topic 名稱、TF 都還是紙上規劃)
...
`ros2 pkg executables`/`ros2 launch --show-args` 也都確認正常。過程中發現並修掉三個真實 bug(細節見
「已知眉角」):
